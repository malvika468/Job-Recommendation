...
def exploreJobs():
	...
	jobdetails = {}
	alljobs = []
	with open('C:\Users\user\Desktop\CF\project\Wuzzuf_Job_Posts_Sample\Wuzzuf_Job_Posts_Sample.csv','rb') as csvfile:
	...
	print "job_to_index:",len(job_to_index)
	...
	pickle.dump(job_to_index,open("job_Cat_to_index.p","wb"))
	pickle.dump(index_to_job,open("index_to_job_Cat.p","wb"))
   

def exploreApplications():
	count=0
	appdetails = {} # userid : [jobids]
	with open('C:\Users\user\Desktop\CF\project\Wuzzuf_Applications_Sample\Wuzzuf_Applications_Sample.csv','rb') as csvfile:
		a = csv.reader(csvfile,delimiter=',')
		for row in a:
			if count == 0:
				count+=1
	...
	#pprint(appdetails)   ## list of user id vs job id he has applied 
